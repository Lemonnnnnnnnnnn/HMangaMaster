...
export class MangaService {
  ...
  async loadImages(mangaPath: string) {
    try {
      // 获取所有图片路径
      const imagePaths = await invoke<string[]>('library_get_manga_images', { path: mangaPath });
  ...
  async deleteAndViewNextManga() {
    if (this.mangaStore.currentMangaIndex >= 0) {
      ...
      try {
        ...
        // 执行删除操作
        const success = await invoke<boolean>('library_delete_manga', { path: this.mangaStore.mangaPath });
